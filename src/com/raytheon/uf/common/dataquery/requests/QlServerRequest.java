...
public class QlServerRequest implements IServerRequest {
    ...
    public String logString() {
    	...
    	String dBase = (database == null)
    			? "null" 
    			: String.format("\"%s\"", database);
    	String qry = ("maps".equals(database) && query.length() > 80) 
    			? query.substring(0, 80) + ("...") 
    			: query;
    	String maxR = (maxResults == null)
    			? "null"
    			: String.format("\"%s\"}", maxResults.toString());

    	buf.append(String.format("{\"reqClass\":\"%s\", \"lang\":\"%s\", \"type\":\"%s\", ", 
    			this.getClass().getName(), lang.toString(), type.toString()));
    	buf.append(String.format("\"database\":%s, \"query\":\"%s\", \"paramMap\":{", dBase, qry));
    	if (paramMap != null) {
        	...
        	for (Map.Entry<String,Object> entry : paramMap.entrySet()) {
        		if (firstTime)
        			...
        			buf.append(", ");
        		buf.append(String.format("\"%s\":\"%s\"", entry.getKey(), entry.getValue().toString()));
        	}
    	}

    	buf.append("}, ");
   		buf.append(String.format("\"maxResults\":%s}", maxR));
