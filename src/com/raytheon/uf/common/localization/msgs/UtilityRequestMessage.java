...
public class UtilityRequestMessage implements IServerRequest {
    ...
    public String logString() {
    	 ...
    	 * {"reqClass":"<class>", "commands":[{<command dict1>}, {<command dict2>}, ...]}
    	 */
        StringBuilder buf = new StringBuilder();
        
        buf.append(String.format("{\"reqClass\":\"%s\", \"commands\": [", this.getClass().getName()));
        for (int i = 0; i < commands.length; i++) {
        	if (i != 0)
        		buf.append(", ");
        	buf.append(String.format("{%s}", commands[i].logString()));
        }
        buf.append("]}");
        return buf.toString();
    }
